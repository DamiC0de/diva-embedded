...
class ParoliDaemon:
    ...
    def synthesize(self, text: str, length_scale: float = 1.0) -> bytes:
        """Send text to daemon and read raw PCM output (non-blocking)."""
        import select, fcntl
        with self.lock:
            ...
            # Read raw PCM with non-blocking IO + select
            pcm_chunks = []
            start = time.time()
            got_data = False
            idle_cycles = 0
            while time.time() - start < SYNTHESIS_TIMEOUT:
                r, _, _ = select.select([self.process.stdout], [], [], 0.05)
                if r:
                    try:
                    ...
                    if got_data:
                        idle_cycles += 1
                        if idle_cycles > 2:  # 100ms silence after data = synthesis done
                            break
...
# Daemon mode enabled — 5x faster than fork (model stays in memory)
daemon = ParoliDaemon() if USE_PAROLI else None
log_json("info", "Using daemon mode" if daemon else "Using fork mode")
