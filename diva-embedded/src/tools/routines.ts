...
interface Routine {
  name: string;
  triggerPhrases: string[];  // e.g. ["routine dodo", "bonne nuit routine"]
...
export function findRoutine(text: string): Routine | null {
  ...
  for (const action of routine.actions) {
    try {
      switch (action.type) {
        ...
        case "timer":
          startTimer(
            Number(action.params.durationMs || 60000),
            String(action.params.label || "routine"),
...
export function listRoutines(): string {
  const routines = loadRoutines();
  if (routines.length === 0) return "Aucune routine configuree.";
  return `Routines disponibles : ${routines.map((r) => r.name).join(", ")}.`;
}

export function getRoutinesForDashboard(): Routine[] {
  return loadRoutines();
}
