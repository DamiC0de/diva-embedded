...
import { getSession, addUserExchange, addAssistantExchange, updateLastIntent, updateSystemState, buildSessionContext, canResumeConversation, getLastTopic, setCorrelationId as setSessionCorrelationId, buildResumptionPrompt, markResumeConsumed, isSecretTrusted, setSecretTrustWindow, clearSecretTrustWindow, recordSpeakerActivity, updateSessionLang } from "./session/session-manager.js";
...
import { FormulationHelper, type SessionContext as FormulationSessionContext } from "./session/formulation-helper.js";
...
import { formatConfirmation, type ConfirmationContext } from "./routing/confirmation-formatter.js";
...
import { chooseFiller, chooseContextualFiller, scheduleFillers, clearFillerHistory, type FillerHandle } from "./audio/filler-manager.js";
...
import { waitForHA, startHAHealthCheck, getAreas, getEntitiesByArea } from "./smarthome/ha-connector.js";
// Story 3.9: Satellite management and room context injection
import { SatelliteManager, DEFAULT_ROOM_ID, type HAConnectorLike } from "./satellite/satellite-manager.js";
import { RoomContextInjector } from "./satellite/room-context-injector.js";
...
let proactiveLoop: ProactiveSchedulerLoop | null = null;
...
import { waitForAdminResponse, executeApprovedAction, getPendingMediation, auditMediation, type MediationResult } from "./household/permission-mediator.js";
...
import { setCurrentRegister, getCurrentAdaptation, resetRegister, type VocalRegister } from "./audio/vocal-register.js";
import { prosodyToInitialMode, fuseMode, setCurrentMode, resetMode, type WakewordProsody } from "./audio/wakeword-prosody.js";
import { suppressNoise } from "./audio/noise-suppressor.js";
import { cancelEcho, markOutputStart, markOutputEnd, isDivaOutputting, getReferenceBuffer, AEC_PIPELINE_ORDER } from "./audio/echo-canceller.js";
...
import { recordVisit, getVisitorType, shouldProposeRegistration, activateInviteMode, deactivateInviteMode, isInviteMode, ensureVisitorSchema, loadVisitorsFromDb, purgeOldVisitors, VISITOR_PURGE_DAYS, getVisitorRecord } from "./persona/visitor-classifier.js";
...
import { recordAction as recordDomoticAction, getProposal, shouldAutoExecute, getActiveAutomations } from "./smarthome/emergent-automations.js";
...
import { isSimpleCommand, buildQwenDomotiquePrompt, parseQwenResponse } from "./smarthome/domotique-router.js";
...
import { loadModes as loadHomeModes, activateModeByName, getActiveMode as getActiveModeV2, getModes as getHomeModes, getModeByName } from "./smarthome/mode-manager.js";
...
import { loadRoutines as loadConversationalRoutines, executeRoutine as executeConversationalRoutine, getRoutineById as getConversationalRoutineById } from "./tools/routine-manager.js";
...
import { canControlDevice, isChildFunCommand, getChildDenialMessage } from "./smarthome/child-access-control.js";
import { checkForAlerts, formatVocalAlert, getActiveAlerts } from "./smarthome/domotique-alerts.js";
import { getWeatherSuggestion } from "./smarthome/meteo-domotique.js";
import { checkDeviceHealth, getLowBatteryDevices } from "./smarthome/device-health.js";
import { checkBadges, getEarnedBadges } from "./smarthome/gamification-eco.js";
...
import { shouldTriggerOOBE, runOOBE, runOOBEReprompt, shouldResumeDiscovery, checkPreConfigBoot, checkLockedSetting } from "./household/oobe-flow.js";
...
import { attemptVoiceLink, confirmVoiceLink } from "./household/voice-linker.js";
// Story 3.4: Speaker resolution chain and foyer-persona sync
import { resolveSpeaker, syncFoyerPersonas, SPEAKER_ID_TIMEOUT_MS } from "./household/speaker-resolver.js";
...
import { shouldUseBridge, generateBridgeResponse, buildBridgeContextTag, shouldSkipBridgeForMode } from "./llm/qwen-bridge.js";
// Story 11.4: Centralized LLM Router
import { route as llmRoute, type RouteDecision } from "./llm/llm-router.js";
import { recordRouteDecision } from "./monitoring/metrics-collector.js";
import { ensureNetworkConnection, restartHotspot, openPostConfigHotspot } from "./onboarding/wifi-manager.js";
import { generateQRImage } from "./onboarding/qr-generator.js";
// Story 22.2 Task 6.1: Boot type detection before all services
import { detectBootType as detectBootTypeForInit, BootType as BootTypeForInit } from "./onboarding/boot-type-detector.js";
import { announceBootType as announceBootForInit } from "./onboarding/setup-announcements.js";
...
import { shouldInsertCallback, recordCallback, getRemainingCallbacks, getCallbackLevelSync, touchSession as touchCallbackSession, warmupCallbackLevel } from "./memory/callback-manager.js";
...
import { recordRationale, getLastRationale, findRationale, clearRationales, type ActionRationale } from "./memory/action-rationale-log.js";
...
const formulationHelper = new FormulationHelper(requestPatternStore, {
    ...
    personaCheck: () => {
        try {
            ...
            return (persona as any).formulationHelp !== false;
        } catch { return true; }
    },
    claudeCallback: FORMULATION_HELP_ENABLED ? async (prompt: string) => {
        try {
...
if (FORMULATION_HELP_ENABLED) {
    hesitationDetector.onHesitation(async (event) => {
        try {
            ...
            const ctx: FormulationSessionContext = {
...
async function init(): Promise<void> {
    ...
    (globalThis as any).__householdMessaging = householdMessaging;
    ...
    try {
      ...
      const msgSender = new MessageSender({
        ...
        auditFn: (speakerId, action, status, details) => {
          try { logAudit(action, "critical", speakerId, status, undefined, details); } catch { /* non-blocking */ }
        },
      });
      ...
      networkDetector.on("network.status-changed", (evt: any) => {
        if (evt.newStatus === "online") {
          queueProc.processQueue().catch((err: Error) => {
      ...
      (globalThis as any).__messageSender = msgSender;
      (globalThis as any).__contactManager = contactMgr;
      (globalThis as any).__queueProcessor = queueProc;
    ...
    claude.registerTool("toggleHumor", async (input) => {
      const enabled = String(input.enabled) === "true" || (input.enabled as unknown) === true;
    ...
    try {
      ...
      ecoGamification.setChildCheck((speakerId: string) => {
    ...
    try {
      ...
      (globalThis as any).__divaTokenTracker = monTokenTracker;
      (globalThis as any).__divaQualityAggregator = monQualityAggregator;
    ...
    setTextInputHandler(async (text: string, speaker: string) => {
    ...
    degradationAnnouncer.setPersonaResolver((speakerId: string) => {
    ...
    try {
        setPreferredLangUpdater((speakerId: string, lang: string) => {
            updatePreferredLang(speakerId, lang);
        });
        setPreferredLangGetter((speakerId: string) => {
...
async function continuePostNetwork(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const speakerIds = personas.map((p: { id: string }) => p.id);
    ...
    try {
      ...
      const ledDriver = await createLedDriver(
        process.env.LED_SPI_DEVICE || "/dev/spidev0.0",
        (process.env.LED_PROTOCOL as "ws2812b" | "apa102") || "ws2812b"
      );
    ...
    try {
      ...
      (globalThis as any).__divaMultimodalWakeManager = multimodalWakeManager;
    ...
    try {
      ...
      try {
        ...
        if (haConn.isHAAvailable()) {
          const states = (await haConn.callHA("states")) as Array<{ entity_id: string; state: string; attributes: Record<string, unknown> }>;
    ...
    try {
      ...
      const speakerIds = foyer && isFoyerConfigured()
        ? getMembers(foyer.id).map((m: { name: string }) => m.name)
        : [];
      initCorrectionTracker(speakerIds, async (query: string) => {
        try {
    ...
    const haToken = (() => { try { return require("node:fs").readFileSync("/opt/diva-embedded/data/.ha-token", "utf-8").trim(); } catch { return ""; } })();
    ...
    setInterval(async () => {
        try {
            ...
            if (haToken) {
                try {
                    ...
                    if (statesResp.ok) {
                        ...
                        const newAlerts = checkForAlerts(haStates as any[]);
                        ...
                        const weatherSuggestion = getWeatherSuggestion(haStates as any[], { current: { temp: 20, humidity: 50 }, forecast: [] } as any);
                        if (weatherSuggestion) {
                            await speakTTS(weatherSuggestion.message);
                        }

                        // Story 18.6: Device health check
                        const lowBattery = getLowBatteryDevices(haStates as any[]);
            ...
            try {
              ...
              if (alerts.length > 0) {
                await speakTTS(String((alerts[0] as any).message || ""));
              }
            } catch {}
        } catch {}
...
function extractNameForCheck(text: string): boolean {
    ...
    if (/je\s+m'appelle|moi\s+c'est|c'est\s+\w+|je\s+suis\s+\w+/.test(lower)) return true;
    return false;
}
...
async function speakTTS(text: string): Promise<void> {
    try {
        ...
        try { getLedStateManager().setState("speaking"); } catch {}
...
const SYNTHESIZE_TIMEOUT_MS = 3000;

/** Story 1.5 / Task 2.4: synthesize with timeout */
function synthesizeWithTimeout(text: string): Promise<Buffer> {
    return Promise.race([
        synthesize(text),
        new Promise<Buffer>((_, reject) =>
            setTimeout(() => reject(new Error("Piper TTS timeout (3s)")), SYNTHESIZE_TIMEOUT_MS)
        ),
    ]);
}

async function speakTTSStreaming(
    sentenceQueue: AsyncIterable<string>,
    correlationId?: string,
    onFirstSentencePlayed?: () => void,
): Promise<void> {
    ...
    let fallbackCollector: string[] = []; // Collect remaining text for fallback
...
async function idleLoop(): Promise<void> {
    ...
    try {
        ...
        try { getLedStateManager().setState("listening"); } catch {}

        // Story 3.11: Route vocal wake-word through MultimodalWakeManager (rétrocompatibilité)
        const multimodalManager = (globalThis as any).__divaMultimodalWakeManager;
        if (multimodalManager) {
            ...
            (voiceWakeEvent as any).correlationId = corrId;
            multimodalManager.handleWakeEvent(voiceWakeEvent);
        }

        // Story 27.1: Detect wake-word position (start/middle/end)
        const wwPosition = detectWakewordPosition(
            wakeword.pre_audio_base64,
            wakeword.post_audio_base64,
        );
        ...
        if (wakeword.wakeword_prosody) {
            const wwProsody: WakewordProsody = {
...
async function conversationLoop(): Promise<void> {
    ...
    while (true) {
        ...
        if (recorded.vocalRegister) {
            setCurrentRegister(recorded.vocalRegister as VocalRegister);
        ...
        let earlyIntentPromise: Promise<any> | null = null;
        ...
        try { getLedStateManager().setState("processing"); } catch {}
            ...
            try {
                ...
                if (returningGreeting) {
                    ...
                    try {
                        ...
                        if (proposal) await speakTTS(proposal);
                    } catch {}
                }
            } catch {}
        ...
        if (!resolution.identified && recorded.wav_base64 && recorded.duration_ms) {
            ...
            if (foyer) {
                ...
                if (preInscrit.length > 0 && backgroundCollector.shouldCollect(getCorrelationId())) {
                    ...
                    backgroundCollector.collectSample(
                        recorded.wav_base64,
        ...
        if (!resolution.identified) {
            ...
            if (
                backgroundCollector.getSampleCount(sessionId) >= 3 &&
        ...
        if (isDistressPhrase(transcription)) {
            ...
            try { getLedStateManager().setState("urgency"); } catch {}
            const response = await handleDistress(transcription);
            await speakTTS(response);
            try { getLedStateManager().clearState("urgency"); } catch {}
            break;
        }
        ...
        if (isFirstTurn && speaker !== "unknown") {
            ...
            if (reprompt?.completed) break; // Reprompt handled the interaction
        }
 ...
 */
async function conversationLoopWithAudio(wavBase64: string): Promise<void> {
    ...
    if (resolution.identified && resolution.source === "foyer") {
        ...
        try {
            const hm = (globalThis as any).__householdMessaging as HouseholdMessaging | undefined;
            if (hm) {
...
async function handleTranscription(transcription: string, speaker: string = "unknown", lastAudioBuffer?: Buffer, speakerConfidenceScore?: number): Promise<void> {
    // Story 3.6 / AC3, AC7: Intercept name response if awaiting visitor name
    if (isAwaitingVisitorName(speaker)) {
        try {
            let visitorDb: import("better-sqlite3").Database | undefined;
            try {
                const { getCompanionDb } = await import("./security/database-manager.js");
                visitorDb = getCompanionDb();
            } catch {}
            const nameResponse = await handleVisitorNameResponse(
                speaker,
                transcription,
                getCorrelationId(),
                visitorDb,
    ...
    if (subRequests.length >= 2) {
        ...
        try {
            const multiResponse = await handleMultiRequest(
                subRequests,
                transcription,
                speaker,
                getCorrelationId(),
                (msg: string) => claude.chat(msg),
            );
            if (multiResponse) {
    ...
    let serviceContextHeader: string | null = null;
    if (SERVICE_CONTEXT_ENABLED) {
      try {
        ...
        if (isSwitch) {
          const persona = getCurrentPersona();
          const svcCtx = buildServiceContext(
            speaker,
            intent.category,
            session,
            activeActionRegistry,
            persona?.greetingName,
          );
          if (svcCtx) {
    ...
    if (!authResult.allowed) {
        ...
        try {
            const mediationResult = await onAuthDenied(speaker, intent.category, (intent as any).params || {}, getCorrelationId());
            ...
            if (mediationResult.type === "redirected" && mediationResult.adminId) {
                ...
                if (adminResponse === "approved" && pending) {
                    ...
                    const fallback = mediate(speaker, intent.category, (intent as any).params || {}, getCorrelationId());
    ...
    if (authResult.needsLivenessCheck && lastAudioBuffer) {
        ...
        if (!liveness.isLive) {
            ...
            auditLivenessFailed(speaker, intent.category, authResult.level as "protected" | "critical", "liveness_failed");
    ...
    if (authResult.needsSecretVerification && speaker !== "unknown") {
        ...
        if (isSecretTrusted(speaker)) {
            ...
            if (!flowResult.success) {
                if (flowResult.message) await speakTTS(flowResult.message);
                return;
            }
    ...
    if (/chaque (matin|soir|jour|lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)|tous les (matins|soirs|jours)|a \d+h/i.test(transcription) && /allume|eteins|ouvre|ferme|lance|demarre|mets|active/i.test(transcription)) {
        ...
        createRoutine(routineName, { type: "vocal" } as any, [{ type: "raw", config: { text: transcription } }] as any, speaker);
    ...
    if (detectedMode) {
        ...
        const modeNames: Record<string, string> = { MAISON: "maison", NUIT: "nuit", ABSENT: "absent" };
    ...
    if (intent.category === "home_control" && routeDecision.engine === "qwen-domotique") {
        ...
        try {
            ...
            const devices: any[] = []; // TODO: fetch from HA cache
            ...
            if (qwenResp) {
                ...
                if (action) {
                    ...
                    if (routeDecision.shouldSpeak) {
                        ...
                        try { await playAudioFile(`${ASSETS_DIR}/confirm-beep.wav`); } catch {}
                    }
    ...
    if (intent.intent === "local") {
        ...
        if (local.handled && local.response) {
            ...
            let finalResponse: string | null = local.response;
            if (IMPLICIT_CONFIRMATIONS) {
                try {
                    const confirmCtx: ConfirmationContext = {
            ...
            if (local.silentAction) {
                ...
                if (feedbackEnabled) {
                    try { await playAudioFile(`${ASSETS_DIR}/click.wav`); } catch {}
                }
            ...
            if (SERVICE_CONTEXT_ENABLED && audioDucker.isDucked()) {
              try { await audioDucker.resume(); } catch {}
            }
    ...
    if (intent.intent !== "local" || !intent.category) {
      try {
        ...
        if (implicitIntent) {
          ...
          // Emit internal event
          process.emit("intent.implicit-detected" as any, {
            type: implicitIntent.type,
            extracted: implicitIntent.extracted,
            speakerId: speaker,
    ...
    let fillerHandle: FillerHandle | null = null;
    ...
    if (shouldInjectPersonalData(speaker, speakerConfidenceScore)) {
        try {
            // Story 6.1 / Task 5.1: Semantic memory recall before Claude call
            const recalledMemories = await recallRelevantMemories(speaker, transcription);
            let memorySummaryText: string;
            ...
            const session = getSession(speaker);
            const exchangeCount = session.exchanges.filter((e: { role: string }) => e.role === "user").length;
            ...
            (claude as any)._story61_callbackLevel = callbackLevel;
            (claude as any)._story61_remaining = canCallback ? remaining : 0;
            (claude as any)._story61_memorySummary = memorySummaryText;
            ...
            try {
                ...
                if (prefs.length > 0) {
                    (claude as any)._story62_preferencesSummary = formatPreferencesForPrompt(prefs);
                }
                const upcoming = await getUpcomingDates(speaker, 7);
                if (upcoming.length > 0) {
                    (claude as any)._story62_upcomingDatesSummary = upcoming
            ...
            try {
                const aliasResult = await resolveConversationalAliases(
                    transcription,
                    speaker,
                    getCorrelationId(),
                );
                const aliasContext = formatAliasContextForPrompt(
                    aliasResult,
                    getCurrentPersona()?.greetingName,
                );
                if (aliasContext) {
                    (claude as any)._story63_aliasContext = aliasContext;
                }
    ...
    try {
      const persona = getCurrentPersona();
      const session = getSession(speaker);
      const memorySummaryText = (claude as any)._story61_memorySummary as string | undefined;
      const humorCtx = buildHumorContext(speaker, persona, session, memorySummaryText, getCorrelationId());
      if (humorCtx) {
        (claude as any)._story64_humorContext = humorCtx;
      }
    } catch (err) {
    ...
    try {
      if (isTransparencyQuestion(transcription)) {
        ...
        (claude as any)._story65_transparencyContext = transparencyCtx;
        ...
        if (lastRat) {
          const mem0Text = processHeuristicCorrection(
            speaker,
            lastRat,
            transcription,
            getCorrelationId() || "",
          ...
          (claude as any)._story65_transparencyContext = formatContestationPrompt();
        }
      }
    } catch (err) {
    ...
    (claude as any)._story114_modeModifier = routeDecision.promptModifier;

    // Story 11.1 / Task 4 + Story 11.4 / Task 4.7: Tell Claude what Qwen already said
    if (bridgeText) {
        claude.setSessionContext(
    ...
    if (backend === "claude") {
        try {
            ...
            if (fullResponse && fullResponse.trim().length > 0) {
                ...
                if (IMPLICIT_CONFIRMATIONS) {
                    try { fallbackText = sanitizeConfirmation(fullResponse, "claude").text; } catch { /* use original */ }
                }
    ...
    try {
      ...
      if (fullResponse && session.lastAction) {
        ...
        const sources: Array<"memory" | "session" | "weather" | "calendar" | "presence" | "wellness" | "proactive"> = [];
...
const shutdown = () => {
    ...
    try { if (proactiveLoop) proactiveLoop.stop(); } catch { /* already stopped */ }
    // Story 16.2: Stop conversational routine scheduler cleanly
    try { stopRoutineScheduler(); } catch { /* already stopped */ }
