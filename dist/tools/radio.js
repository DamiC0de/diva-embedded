...
function findStation(text) {
    ...
    // Fuzzy match
    if (/inter\b/i.test(lower))
        return { key: "france inter", ...RADIO_STREAMS["france inter"] };
    if (/info/i.test(lower))
        return { key: "france info", ...RADIO_STREAMS["france info"] };
    if (/culture/i.test(lower))
        return { key: "france culture", ...RADIO_STREAMS["france culture"] };
    if (/classique/i.test(lower))
        return { key: "classique", ...RADIO_STREAMS["classique"] };
    if (/jazz/i.test(lower))
        return { key: "jazz", ...RADIO_STREAMS["jazz"] };
    if (/nostalg/i.test(lower))
        return { key: "nostalgie", ...RADIO_STREAMS["nostalgie"] };
    return null;
}
export function playRadio(text) {
...
export function stopRadio() {
    ...
    try {
        execSync("pkill -f 'mpv.*icecast\\|mpv.*streaming\\|mpv.*ice.infomaniak\\|mpv.*scdn.nrj\\|ffplay.*icecast' || true", { timeout: 2000 });
    }
    catch { }
    const wasPlaying = currentStation;
    currentStation = null;
    if (wasPlaying)
        return `J'ai arrete ${wasPlaying}.`;
...
export function setVolume(text) {
    ...
    if (numMatch) {
        ...
        try {
            ...
            return "Impossible de couper le son.";
        }
    }
