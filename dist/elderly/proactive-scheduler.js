...
export function trackRepeatedQuestion() {
    dailyRepeatedQuestions++;
}
export function addWellnessNote(note) {
...
async function announceTime() {
    ...
    let context = "";
    if (h >= 7 && h < 9)
        context = " C'est l'heure du petit-dejeuner.";
    else if (h >= 12 && h < 13)
        context = " C'est bientot l'heure du dejeuner.";
    else if (h >= 18 && h < 19)
        context = " C'est bientot l'heure du diner.";
    await speakProactive(`Il est ${timeStr}. Nous sommes ${dayStr}.${context}`);
}
// --- Companionship Check-in ---
async function companionshipCheckin() {
    const personas = listPersonas();
    const elderlyPersonas = personas.filter((p) => p.type === "elderly" || p.type === "alzheimer");
    if (elderlyPersonas.length === 0)
        return;
