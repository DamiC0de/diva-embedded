...
export class AecService extends EventEmitter {
  ...
  private async createFifos(): Promise<void> {
    ...
    for (const fifo of [EC_INPUT_FIFO, EC_OUTPUT_FIFO]) {
      ...
      await new Promise<void>((resolve, reject) => {
        proc.on("close", (code) =>
          code === 0 ? resolve() : reject(new Error(`mkfifo ${fifo} failed`))
        );
      });
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    for (const fifo of [EC_INPUT_FIFO, EC_OUTPUT_FIFO]) {
      if (existsSync(fifo)) await unlink(fifo).catch(() => {});
    }
    this.process = null;
  ...
  async writeSpeaker(data: Buffer): Promise<void> {
