import { readdir, readFile, writeFile, mkdir, appendFile, stat } from "node:fs/promises";
...
export class MemoryManager {
  ...
  private conversationMessages: ConversationEntry[] = [];
   ...
   */
  async read(userId: string = "default"): Promise<MemoryEntry[]> {
   ...
   * @param userId - User identifier
   */
  async search(query: string, userId: string = "default"): Promise<MemoryEntry[]> {
   ...
   * @param userId - User identifier
   */
  async getSummary(userId: string = "default"): Promise<string> {
    const entries = await this.read(userId);
  ...
  private async persistMessage(entry: ConversationEntry): Promise<void> {
    await this.ensureDirs();
