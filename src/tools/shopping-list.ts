...
function saveList(items: ShoppingItem[]): void {
  writeFileSync(LIST_PATH, JSON.stringify(items, null, 2));
}

export function addItem(name: string, addedBy: string = "default"): string {
...
export function removeItem(name: string): string {
  const items = loadList();
  const normalized = name.trim().toLowerCase();
...
export function clearList(): string {
  saveList([]);
  return "Liste de courses videe.";
}

export function getListItems(): ShoppingItem[] {
  return loadList();
}
 ...
 */
export function handleShoppingCommand(text: string, speaker: string = "default"): { handled: boolean; response?: string } {
