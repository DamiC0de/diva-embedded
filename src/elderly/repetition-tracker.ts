...
const recentQuestions: Array<{ text: string; timestamp: number }> = [];
const MAX_RECENT = 50;
 ...
 */
export function checkRepetition(text: string): { isRepetition: boolean; count: number } {
  ...
  for (const recent of recentQuestions) {
    if (recent.timestamp < oneHourAgo) continue;
...
function logRepetition(text: string): void {
  try {
    const log: RepetitionEntry[] = existsSync(TRACKER_PATH)
...
export function getRepetitionStats(days: number = 7): { totalRepetitions: number; uniqueQuestions: number; entries: RepetitionEntry[] } {
  try {
    if (!existsSync(TRACKER_PATH)) return { totalRepetitions: 0, uniqueQuestions: 0, entries: [] };
    const log: RepetitionEntry[] = JSON.parse(readFileSync(TRACKER_PATH, "utf-8"));
    const cutoff = new Date();
