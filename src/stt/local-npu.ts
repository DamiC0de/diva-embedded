...
function cleanHallucinations(text: string): string {
  ...
  if (lower.length <= 4 && !VALID_SHORT_WORDS.has(lower)) return "";

  const repetitionMatch = trimmed.match(/(.{5,}?)\1{2,}/);
  if (repetitionMatch) return repetitionMatch[1].trim();
...
async function transcribeNPU(wavBuffer: Buffer): Promise<string> {
  ...
  if (!response.ok) throw new Error(`Local STT error ${response.status}`);
...
async function transcribeGroq(wavBuffer: Buffer): Promise<string> {
...
export async function transcribeLocal(wavBuffer: Buffer): Promise<string> {
  try {
    const rawText = await withCircuitBreaker(
      "stt",
      () => transcribeNPU(wavBuffer),
      () => transcribeGroq(wavBuffer),
